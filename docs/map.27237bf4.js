...
var g = svg.append('g');
var projection = d3.geoConicConformal().parallels([47 + 30 / 60, 48 + 44 / 60]).rotate([120 + 0 / 60, 0]).scale(1000);
...
function ready(data) {
  ...
  projection.scale(1).translate([0, 0]); // Book: Learning D3.js 4 Mapping Second Edition
  ...
  g.append('g').selectAll('class', 'state').data(state).enter().append('path').attr('d', path).attr('class', 'states');
