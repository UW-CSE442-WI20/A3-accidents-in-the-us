...
temps = []
for index, row in data.iterrows():
    temps.append(myround(float(row['Temperature(F)'])))


data['Temperature'] = temps
data['Latitude'] = data['Start_Lat']
data['Longitude'] = data['Start_Lng']

data = data[['Severity', 'Temperature', 'General_Time', 'Latitude', 'Longitude']]
data.to_csv("WA.csv")